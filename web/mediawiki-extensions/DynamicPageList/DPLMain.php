...
class DPLMain {
	...
	*/
	...
	public static function dynamicPageList( $input, $params, &$parser, &$bReset, $calledInMode ) {
		...
		$sFirstRevisionBefore = ExtDynamicPageList::$options['firstrevisionbefore']['default'];
			...
			switch ( $sType ) {
				...
				case 'allrevisionssince':
					if ( preg_match( ExtDynamicPageList::$options[$sType]['pattern'], $sArg ) ) {
						...
						if ( ( $sType ) == 'firstrevisionbefore' ) {
							$sFirstRevisionBefore = $date;
						}
		...
		if ( $bAddEditDate && !array_intersect( $aOrderMethods, array( 'firstedit', 'lastedit' ) ) &
			( $sLastRevisionBefore . $sAllRevisionsBefore . $sFirstRevisionSince . $sFirstRevisionBefore . $sAllRevisionsSince == '' )
		) {
		...
		*/
		...
		if ( $sLastRevisionBefore . $sAllRevisionsBefore . $sFirstRevisionSince . $sFirstRevisionBefore . $sAllRevisionsSince != '' ) {
			...
			if( $sFirstRevisionBefore != '' ) {
				$sSqlCond_page_rev .= ' AND ' . $sPageTable . '.page_id=rev.rev_page AND rev.rev_timestamp=( SELECT MIN(rev_aux_snc.rev_timestamp) FROM ' . $sRevisionTable . ' AS rev_aux_snc WHERE rev_aux_snc.rev_page=rev.rev_page AND rev_aux_snc.rev_timestamp <= ' . $sFirstRevisionBefore . ')';
			}
